  ...
  methods:{
    ...
    retrieveData(url) {
      try {
        ...
        this.$axios
          .get(`${process.env.apiBaseUrl}panel/datatables/${urlParam}?table=exercise_categories_file&page=${params.page}&per_page=${params.size}&search=${params.title}&search_columns=name,email,phone&where_column=exercise_category_id&where_value=${this.inputData.id}&joins=exercise_categories_file`)
          .then(response => {
