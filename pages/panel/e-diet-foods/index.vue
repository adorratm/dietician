		...
		<div class="page-wrapper">
			<v-container fluid class="content">
				<div class="page-header">
					<h3 class="page-title">E- Diyet Besinleri</h3>
					<ul class="breadcrumb">
						<li class="breadcrumb-item">
							<nuxt-link to="/panel">Anasayfa</nuxt-link>
						</li>
						<li class="breadcrumb-item active">E- Diyet Besinleri</li>
					</ul>
				</div>
				<v-card tile>
	...
	export default {
		...
		methods: {
			...
			retrieveData(url) {
				...
				this.$axios
					.get(
						`${process.env.apiBaseUrl}panel/e-diet-foods/${urlParam}?table=edietfoods&page=${params.page}&per_page=${params.size}&search=${params.title}&search_columns=name,description`,
						{
							json: true,
			...
			getDisplayData(data) {
				return {
					...
					img_url:
						this.img_url +
						(data.edietfoods !== null &&
						data.edietfoods !== undefined &&
						data.edietfoods !== "" &&
						data.edietfoods.img_url !== "" &&
						data.edietfoods.img_url !== undefined &&
						data.edietfoods.img_url !== null
							? data.edietfoods.img_url
							: this.empty_url),
