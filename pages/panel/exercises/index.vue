		...
		<div class="page-wrapper">
			<v-container fluid class="content">
				<div class="page-header">
					<h3 class="page-title">Egzersizler</h3>
					<ul class="breadcrumb">
						<li class="breadcrumb-item">
							<nuxt-link to="/panel">Anasayfa</nuxt-link>
						</li>
						<li class="breadcrumb-item active">Egzersizler</li>
					</ul>
				</div>
				<v-card tile>
	...
	export default {
		...
		methods: {
			...
			retrieveData(url) {
				...
				this.$axios
					.get(
						`${process.env.apiBaseUrl}panel/exercises/${urlParam}?table=exercises&page=${params.page}&per_page=${params.size}&search=${params.title}&search_columns=name`,
						{
							json: true,
			...
			editData(id) {
				this.$router.push("/panel/exercises/update/" + id);
			},
			deleteData(id) {
				this.$axios
					.delete(process.env.apiBaseUrl + "panel/exercises/delete/" + id, {
						json: true,
						withCredentials: false,
						mode: "no-cors",
						headers: {
				...
				this.$axios
					.get(
						process.env.apiBaseUrl +
							"panel/datatables/is-active-setter?table=exercises&id=" +
							id,
						{
							json: true,
							withCredentials: false,
			...
			getDisplayData(data) {
				return {
					...
					img_url:
						...
						data.exercises !== "" &&
						data.exercises.img_url !== "" &&
						data.exercises.img_url !== undefined &&
						data.exercises.img_url !== null
							? data.exercises.img_url
							: this.empty_url),
