      ...
      <v-card class='mb-3 text-center'>
        ...
        <v-card-text>
          ...
          <v-text-field
            ...
            @change='search()'
            v-model='searchText'
            placeholder='Egzersiz Kategorisi Arayın'
          />
        </v-card-text>
      </v-card>
        ...
        <v-card-text v-else>
          <v-row v-if='exercisecategories.length>0'>
            <client-only>
              ...
              >
                <v-card>
                  <nuxt-link v-bind:to='/exercises/ + exercisecategory.slug'>
                    <v-img
                      height='300'
                      :aspect-ratio='16/9'
                      :alt='exercisecategory.name'
                      :src='exercisecategory.img_url'
                      :lazy-src='exercisecategory.img_url'
                    />
                  </nuxt-link>
                  <v-card-title>
                    <nuxt-link class="purple--text lighten-2" v-bind:to='/exercises/ + exercisecategory.slug'>{{
                  ...
                  <v-card-actions>
                    <v-btn
                      v-bind:to='/exercises/ + exercisecategory.slug'
                      color='purple'
                      block
                      class="white--text"
                    >Tüm Egzersizler
...
export default {
  ...
  methods: {
    async search() {
      this.loading=true
      this.searchText.length > 0 ? this.searchResultText=this.searchText+" ile ilgili Egzersiz Kategorileri" : this.searchResultText='Tüm Egzersiz Kategorileri'
  ...
  watch: {
    ...
    async page(){
      this.loading=true
      await this.$axios.get(process.env.apiBaseUrl+'home/exercisecategories?page='+this.page).then(res=>{
        this.setExercisecategories(res.data.data.data,res.data.data.links.length-2)
      })
      this.loading=false
    }
